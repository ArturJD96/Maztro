...
class Correlations_in_kern_repository:
	...
	def run_query_using_cmd_and_create_files_with_results (self, kern_piece_dir:str):
		...
		with\
			...
			# THIS IS WRONG: should contain -d " " but I have problem with this whitespace.
			subprocess.run(['cleave','-i','**kern','-o','**kern',kern_only_dir], stdout=piece_one_kern_spine) # concatenate every **kern spines into one **kern spine containing chords only.
	...
	def get_correlated_bar_numbers (self):
		...
		with\
			open('query/piece.semits') as sonata_left_column_semits,\
			open('query/piece.correl') as sonata_left_column_correl:
