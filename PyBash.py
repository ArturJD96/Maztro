...
if platform.platform() == 'Windows-10-10.0.18363-SP0':
	humdrum_dir = '/home/bjorn/humdrum-tools'
	os.environ['PATH'] += f':{humdrum_dir}/humdrum/bin'	# is "humdrum/bin" compulsory?
	input_file_name = "/mnt/b/Users/Bjorn/Documents/testikel/input.txt"
	wsl = True
else:
	humdrum_dir = '/usr/local/humdrum-tools/humdrum'
	os.environ['PATH'] += f':{humdrum_dir}'
	input_file_name = '/Users/Arturjds/Desktop/input.krn'

every_sonata_bar_correlation = []

for sonata_file_name in os.listdir(f"{humdrum_dir}/data/mozart/piano-sonatas/kern"):
# if True:
# 	sonata_file_name = 'sonata01-1.krn'
	with\
		open(os.path.join(f"{humdrum_dir}/data/mozart/piano-sonatas/kern", sonata_file_name), 'r') as file,\
		...
		subprocess.run(['semits', '-x', f'{humdrum_dir}/data/mozart/piano-sonatas/kern/{sonata_file_name}'], stdout=sonata_semits)
	...
	with\
		...
		for i, line in enumerate(sonata_left_column_correl):
			...
			try:
				...
				pass
			if correlation > CORRELATION_MINIMUM:
				...
				while not barline_found:
					...
					if line_with_barline.startswith('='):
						...
						try:
							...
							bar_numbers_with_high_correlations.append(i)
						except ValueError:
							pass
